...
async function main() {
  ...
  const nombre = 'RuTAB_Logístico';
  const correo = 'logistico@rutab.com';
   ...
   */
  const logistico = await prisma.administradores.upsert({
    where: { correo: correo },
    update: {},
    create: {
      nombre: nombre,
  ...
  console.log('✅ Logístico de prueba creado con éxito:');
  console.log(`Correo: ${logistico.correo}`);
