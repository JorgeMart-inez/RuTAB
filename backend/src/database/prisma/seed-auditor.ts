...
async function main() {
  ...
  const nombre = 'RuTAB_Auditor';
  const correo = 'auditor@rutab.com';
   ...
   */
  const auditor = await prisma.administradores.upsert({
    where: { correo: correo },
    update: {},
    create: {
      nombre: nombre,
  ...
  console.log('✅ Auditor de prueba creado con éxito:');
  console.log(`Correo: ${auditor.correo}`);
